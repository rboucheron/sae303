...
<div class="w-1/2 lg:w-3/4 m-auto flex flex-col place-content-center">
    <?php if (isset($_SESSION['nom']) && isset($_SESSION['prenom']) && isset($_SESSION['naissance']) && isset($_SESSION['civilite'])) {

    ?>
        <div class="p-2 w-full  lg:w-1/4 text-center border-2 border-cyan-100 cursor-pointer " onclick="openForm()">Modifier ses informations</div>
    <?php
    }
    ?>
    <form action="" method="get">
        <button type="submit" name="profil" value="disconnect" class="p-2 w-full lg:w-1/4 text-center bg-cyan-100 w-1/4 mt-2">Se déconnecter</button>
    </form>
</div>